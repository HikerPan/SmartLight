...
esp_err_t set_str_in_nvs(const char *key, const char *str_value)
{
    ...
    ESP_LOGI(TAG,"nvs %s open success",&current_namespace[0]);
   
    err = nvs_set_str(nvs, key, str_value);
    ESP_LOGI(TAG,"nvs_set_str run");
    if (err == ESP_OK)
    {
        err = nvs_commit(nvs);
        ESP_LOGI(TAG,"nvs_commit run");
        if (err == ESP_OK)
        {
            ESP_LOGI(TAG, "Value %s stored under key '%s'", str_value,key);
        }
    }
