...
static size_t _rym_putchar(struct rym_ctx *ctx, uint8_t code)
{
    size_t len = uart_write_bytes(CONFIG_ESP_CONSOLE_UART_NUM, &code, sizeof(uint8_t));
...
{
    uint8_t byte;
    uart_read_bytes(CONFIG_ESP_CONSOLE_UART_NUM, &byte, 1, pdMS_TO_TICKS(1000));
    return byte;
}
    ...
    uint8_t index)
{
    ...
    do
    {
        _rym_putchar(NULL, ctx->buf[writelen]);
        writelen++;
    }
    ...
    int tm_sec)
{
    ...
    while (xTaskGetTickCount() <= (tick + 100) && i < (data_sz - 1))
    {
...
static int _rym_do_fin(struct rym_ctx *ctx)
{
    ...
    // TODO: 在PSR项目中没有这两个的主动发出，
    // 这两个0x4F不发出，Xshell的ymodem界面不会退出
    // 但是PSR为什么会在互动发出这两个0x4F呢？
    // 可能跟shell软件有关系，后续再思考
    _rym_putchar(ctx, 0x4F);
    _rym_putchar(ctx, 0x4F);
    return 0;
}
#if 1
    ...
    int handshake_timeout)
{
    ...
    /*握手*/
    // ESP_LOGI(TAG, "ready to handshake");
    err = _rym_do_handshake(ctx, handshake_timeout);
    if (err != 0)
    {
        ESP_LOGE(TAG,"ymodem hand shake fail");
        free(ctx->buf);
        return err;
    }
